...
def test_sigmoid_pairwise_hess_grad_one_thread():
    ...
    grad, hess = sigmoid_pairwise_diff_hess(y_true, np.exp(y_pred), 1)

    assert np.sum(np.abs(grad_true - grad)) < 1e-5
    assert np.sum(np.abs(hess_true - hess)) < 1e-5


def test_sigmoid_pairwise_diff_hess_two_thread():
    """Test hess and grad function in two threads"""
    ...
    grad, hess = sigmoid_pairwise_diff_hess(y_true, np.exp(y_pred), 2)

    assert np.sum(np.abs(grad_true - grad)) < 1e-5
    assert np.sum(np.abs(hess_true - hess)) < 1e-5


def test_sigmoid_pairwise_diff_hess_all_zeros_except_one_one_thread():
    """Test hess and grad with all zeros in y_true except one"""

    y_true = np.array([1, 0, 0,  0])
    y_pred = [np.log(3), np.log(1), np.log(1), np.log(1)]
...
def test_sigmoid_pairwise_diff_hess_all_zeroes_except_one_four_thread():
    """Test hess and grad with all zeros in y_true except one 4 threads"""
    ...
    grad, hess = sigmoid_pairwise_diff_hess(y_true, np.exp(y_pred), 4)
...
def test_sigmoid_pairwise_diff_hess_all_zeroes_big_chunk_ten_threads():
    """Test all zeroes with all zeroes in y_pred in one thread"""
    ...
    grad, hess = sigmoid_pairwise_diff_hess(y_true, np.exp(y_pred), 10)

    assert np.sum(np.abs(grad_true - grad)) < 1e-5
    assert np.sum(np.abs(hess_true - hess)) < 1e-5


def test_sigmoid_pairwise_diff_hess_all_zeroes_big_chunk_fifty_threads():
    """Test all zeroes with all zeroes in y_pred in one thread"""
    ...
    grad, hess = sigmoid_pairwise_diff_hess(y_true, np.exp(y_pred), 50)
