...
def get_non_unique_labels_count(int_or_long [::1] y_true, 
    ...
    for r_pointer in range(len(y_pred)):
        ...
        if r_pointer + 1 !=  len(y_pred) and y_pred[y_pred_argsorted[r_pointer]] == y_pred[y_pred_argsorted[r_pointer + 1]]:
            ...
            counter_n = 0
            counter_p = 0
    return np.asarray(counters_p), np.asarray(counters_n)
...
def get_non_unique_borders(float_or_double [::1] y_pred,
    ...
    for j in range(len(y_pred_argsorted)):
        s = len(y_pred_argsorted) - j - 1
        if j + 1 != len(y_pred_argsorted) and y_pred[y_pred_argsorted[s]] == y_pred[y_pred_argsorted[s - 1]]:
    ...
    for j in range(len(y_pred_argsorted) - 1, -1, -1):
        s = len(y_pred_argsorted) - j - 1
        if j - 1 != -1 and y_pred[y_pred_argsorted[s]] == y_pred[y_pred_argsorted[s + 1]]:
    ...
    return np.asarray(y_pred_left), np.asarray(y_pred_right)
...
def get_inverse_argsort(int_or_long [::1] y_true, 
    ...
    for k in range(len(y_pred_argsorted)):
        y_pred_ranks[y_pred_argsorted[y_pred_argsorted.shape[0] -  k - 1]] = k
    return np.asarray(y_pred_ranks)
...
def get_labelscount_borders(int_or_long [::1] y_true, 
    ...
    return np.asarray(counters_p),\
           np.asarray(counters_n),\
           np.asarray(y_pred_left),\
           np.asarray(y_pred_right)